...
const Partie6_apropos: React.FC = () => {
  ...
  const [posteSelected, setPosteSelected] = useState<string>("");
  const [openPosteDropdown, setOpenPosteDropdown] = useState(false);
        ...
        <form className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            ...
            <div className="relative">
              ...
              <button
                type="button"
                onClick={() => setOpenPosteDropdown(!openPosteDropdown)}
                className="mt-2 w-full text-left rounded-lg border border-gray-200 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white flex justify-between items-center hover:bg-gray-50 transition"
              >
                <span className={posteSelected ? "text-gray-900" : "text-gray-400"}>
                  {posteSelected || "Sélectionnez un poste"}
                </span>
                <svg
                  className={`w-4 h-4 transition-transform text-gray-500 ${
                    openPosteDropdown ? "rotate-180" : ""
                  }`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  ...
                  <div className="max-h-60 overflow-y-auto">
                    {postesDisponibles.map((poste) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setPosteSelected(poste);
                          setOpenPosteDropdown(false);
                        }}
                        className="w-full text-left px-4 py-3 text-sm hover:bg-blue-50 transition text-gray-700 font-medium"
                      >
                        {poste}
                      </button>
                    ))}
